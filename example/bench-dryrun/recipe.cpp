...
cell_size_type bench_tile::num_cells() const {
    return params_.num_cells;
}

cell_size_type bench_tile::num_tiles() const {
    return params_.num_tiles;
}

arb::util::unique_any bench_tile::get_cell_description(cell_gid_type gid) const {
...
cell_kind bench_tile::get_cell_kind(cell_gid_type gid) const {
    return cell_kind::benchmark;
}

std::vector<arb::cell_connection> bench_tile::connections_on(cell_gid_type gid) const {
    ...
    std::uniform_int_distribution<cell_gid_type> dist(0, params_.num_cells*params_.num_tiles - 2);

    for (unsigned i=0; i<n; ++i) {
        // Draw random source and adjust to avoid self-connections if necessary.
...
cell_size_type bench_tile::num_targets(cell_gid_type gid) const {
...
cell_size_type bench_tile::num_sources(cell_gid_type gid) const {
    return 1;
}
