    ...
    probe_association_map<probe_handle>& probe_map)
{
    ...
    matrix_ = matrix<backend>(D.parent_cv, D.cell_cv_bounds, D.cv_capacitance, D.face_conductance, D.cv_area, context_.gpu);
    sample_events_ = sample_event_stream(ncell, context_.gpu);
    ...
    state_ = std::make_unique<shared_state>(ncell, D.cv_to_cell, data_alignment? data_alignment: 1u, context_);
