    ...
    cudaStream_t* stream)
{
    if (size>0) {
        ...
        kernel::test_thresholds_impl<<<grid_dim, block_dim, 0, *stream>>>(
    ...
    cudaStream_t* stream)
{
    if (size>0) {
        constexpr int block_dim = 128;
        const int grid_dim = impl::block_count(size, block_dim);
        kernel::reset_crossed_impl<<<grid_dim, block_dim, 0, *stream>>>(size, is_crossed, cv_index, values, thresholds);
    }
}
