...
void init_concentration_impl(
    ...
    const fvm_value_type* weight_Xo, fvm_value_type c_int, fvm_value_type c_ext, cudaStream_t* stream)
{
    if (!n) return;

    constexpr int block_dim = 128;
    int nblock = block_count(n, block_dim);
    kernel::init_concentration_impl<<<nblock, block_dim, 0, *stream>>>(n, Xi, Xo, weight_Xi, weight_Xo, c_int, c_ext);
}
    ...
    fvm_value_type dt, fvm_value_type tmax, cudaStream_t* stream)
{
    ...
    kernel::update_time_to_impl<<<nblock, block_dim, 0, *stream>>>(n, time_to, time, dt, tmax);
}
    ...
    cudaStream_t* stream)
{
    ...
    kernel::vec_minus<<<nblock, block_dim, 0, *stream>>>(ncell, dt_cell, time_to, time);

    nblock = block_count(ncomp, block_dim);
    kernel::gather<<<nblock, block_dim, 0, *stream>>>(ncomp, dt_comp, dt_cell, cv_to_cell);
}
    ...
    cudaStream_t* stream)
{
    ...
    kernel::take_samples_impl<<<nblock, block_dim, 0, *stream>>>(s, time, sample_time, sample_value);
}
